...
namespace discordpp
{
  ...
  inline void RunCallbacks() { Discord_RunCallbacks(); }
  ...
  /// \brief Enum that represents the possible types of relationships that can exist between two
  /// users
  ...
  ///
  /// Generally a user is online or offline, but in Discord users are able to further customize
  /// their status such as turning on "Do not Disturb" mode or "Dnd" to silence notifications.
  ...
  /// \brief Enum that represents various informational disclosures that Discord may make to users,
  /// so that the game can identity them and customize their rendering as desired.
  ///
  ...
  /// \brief Represents the various identity providers that can be used to authenticate a
  /// provisional account user for public clients.
  ...
  /// that user. The SDK will parse those messages for you automatically, and this struct contains
  /// all of the relevant invite information which is needed to later accept that invite.
  class ActivityInvite
  {
    /// \cond
  ...
  ///
  /// While RichPresence supports multiple types of activities, the only activity type that is
  /// really relevant for the SDK is ActivityTypes::Playing. Additionally, the SDK will only expose
  ...
  /// half, `In a group` is powered by the `state` field in the activity, and the second half, `(2
  /// of 3)` is powered by the `party` field in the activity and describes how many people are in
  /// the current party and how big the party can get.
  ///
  ...
  /// 3. ActivitySecrets::Join must be set to a non-empty string. The join secret is only shared
  /// with users who are accepted into the party by an existing member, so it is truly a secret. You
  /// can use this so that when the user is accepted your game knows how to join them to the party.
  /// For example it could be an internal game ID, or a Discord lobby ID/secret that the client
  /// could join.
  ///
  ...
  /// Users can also request to join each others parties. This code snippet shows how that flow
  /// might look:
  ///
  /// \code
  /// // User A
  ...
  /// \brief A VoiceStateHandle represents the state of a single participant in a Discord voice
  /// call.
  ///
  ...
  /// a reference to a handle object, note that it will return the default value for all method
  /// calls (ie an empty string for methods that return a string).
  class VoiceStateHandle
  {
    /// \cond
  ...
  /// \brief Settings for the void auto detection threshold for picking up activity from a user's
  /// mic.
  class VADThresholdSettings
  {
    /// \cond
  ...
  public:
    ...
    using OnStatusChanged = std::function<void(discordpp::Call::Status status,
                                               discordpp::Call::Error error, int32_t errorDetail)>;
    /// \cond
  ...
  /// contain the ID of the channel a message was sent in, and Client::GetChannelHandle will return
  ...
  /// or RelationshipType::PendingOutgoing (based on whether they are receiving or sending the
  /// invite respectively), and their RelationshipHandle::GameRelationshipType would remain as
  ...
  /// information which includes both whether they are online/offline/etc as well as whether they
  ...
  /// any lobby messages? It is possible for a game developer to add a user to a lobby while they
  /// are offline. Also users may temporarily disconnect and rejoin later. So the `Connected`
  /// boolean tells you whether the user is actively connected to the lobby.
  ///
  /// Handle objects in the SDK hold a reference both to the underlying data, and to the SDK
  /// instance. Changes to the underlying data will generally be available on existing handles
  /// objects without having to re-create them. If the SDK instance is destroyed, but you still have
  ...
  /// disconnect. When the SDK connects, it will attempt to re-connect to any lobbies it is
  ...
  /// and status is set to Ready). Additionally, lobbies that are linked to a channel on Discord
  /// will not be auto deleted.
  ///
  /// You can also use the @ref server_apis to customize this timeout, it can be raised to as high
  /// as 7 days, meaning the lobby only gets deleted if no one connects to it for an entire week.
  /// This should give a good amount of permanence to lobbies when needed, but there may be rare
  /// cases where a lobby does need to be "rebuilt" if everyone is offline for an extended period.
  ///
  /// # Membership Limits
  ...
  /// the server APIs, which allows you to ensure that only clan/guild/group leaders can link
  /// lobbies to Discord channels.
  ///
  ...
  ///
  /// It's not really possible for the game to know which users should have access to that channel
  /// or not though. So in this implementation, every member of a lobby will be able to view all
  ...
  /// isViewableAndWriteableByAllMembers boolean which is described more in GuildChannel. You can
  /// use that to just not allow private channels to be linked, or to know when to show a clear
  ...
  /// ChannelHandle::Type methods. The SDK should only be receiving messages in the following
  /// channel types:
  /// - DM
  /// - Ephemeral DM
  /// - Lobby
  ...
  /// message will show up on the Discord client, the SDK will inject a "fake" message into the
  /// chat, that contains a basic English explanation of what is happening to the user. You can
  /// identify these messages with the MessageHandle::DisclosureType method. We encourage you to
  /// customize the rendering of these messages, possibly changing the wording, translating them,
  /// and making them look more "official". You can choose to avoid rendering these as well.
  ///
  /// # History
  ...
  ///
  /// See @ref getting_started "Getting Started" for more information on how to use the Client
  /// class.
  class Client
  {
    /// \cond
  ...
  public:
    ...
    std::function<void(uint64_t userId, int16_t const* data, uint64_t samplesPerChannel,
                       int32_t sampleRate, uint64_t channels, bool& outShouldMute)>;

    /// \brief Callback function for Client::StartCallWithAudioCallbacks.
    using UserAudioCapturedCallback = std::function<void(
    int16_t const* data, uint64_t samplesPerChannel, int32_t sampleRate, uint64_t channels)>;
    ...
    std::function<void(discordpp::ClientResult result, std::string code, std::string redirectUri)>;
    ...
    using TokenExchangeCallback = std::function<void(
    discordpp::ClientResult result, std::string accessToken, std::string refreshToken,
    discordpp::AuthorizationTokenType tokenType, int32_t expiresIn, std::string scopes)>;
    ...
    using OnStatusChanged = std::function<void(
    ...
    using GetGuildChannelsCallback = std::function<void(
    discordpp::ClientResult result, std::vector<discordpp::GuildChannel> guildChannels)>;
    ...
    std::function<void(discordpp::ClientResult result, std::optional<discordpp::UserHandle> user)>;
    ...
    uint64_t lobbyId, discordpp::Client::UserAudioReceivedCallback receivedCb,
    ...
    void FetchCurrentUser(discordpp::AuthorizationTokenType tokenType, std::string const& token,
    ...
    /// the OAuth2 page for your application in the Discord developer portal,
    /// https://discord.com/developers/applications
    void GetToken(uint64_t applicationId, std::string const& code, std::string const& codeVerifier,
    ...
    discordpp::AuthenticationExternalAuthType externalAuthType,
    std::string const& externalAuthToken, discordpp::Client::TokenExchangeCallback callback);
    ...
    /// the OAuth2 page for your application in the Discord developer portal,
    /// https://discord.com/developers/applications
    void GetTokenFromProvisionalMerge(uint64_t applicationId, std::string const& code,
    ...
    /// the OAuth2 page for your application in the Discord developer portal,
    /// https://discord.com/developers/applications
    void RefreshToken(uint64_t applicationId, std::string const& refreshToken,
                      discordpp::Client::TokenExchangeCallback callback);
    ...
    void UpdateToken(discordpp::AuthorizationTokenType tokenType, std::string token,
    ...
    void DeleteUserMessage(uint64_t recipientId, uint64_t messageId,
    ...
    void EditUserMessage(uint64_t recipientId, uint64_t messageId, std::string const& content,
    ...
    /// Discord.
    void SendLobbyMessage(uint64_t lobbyId, std::string const& content,
    ...
    /// message.
    void SendLobbyMessageWithMetadata(uint64_t lobbyId, std::string const& content,
    ...
    void SendUserMessage(uint64_t recipientId, std::string const& content,
                         discordpp::Client::SendUserMessageCallback cb);
    ...
    /// An example use case for this might be to include the name of the character that sent a
    /// message.
    void SendUserMessageWithMetadata(uint64_t recipientId, std::string const& content,
                                     std::unordered_map<std::string, std::string> const& metadata,
                                     discordpp::Client::SendUserMessageCallback cb);
    ...
    std::string const& secret, std::unordered_map<std::string, std::string> const& lobbyMetadata,
    ...
    void LinkChannelToLobby(uint64_t lobbyId, uint64_t channelId,
    ...
    void SendActivityInvite(uint64_t userId, std::string const& content,
...
namespace discordpp
{
  ...
  inline bool HasSynchronizationContext() { return !!s_synchronizationContext; }
  ...
  template <typename T> struct TDelegateUserData
  {
    T delegate;
  ...
  struct ConvertedProperties
  {
    ConvertedProperties(std::unordered_map<std::string, std::string> const& PropertyMap)
    {
      ...
      Properties.keys =
      reinterpret_cast<Discord_String*>(Discord_Alloc(Properties.size * sizeof(Discord_String)));
      Properties.values =
      reinterpret_cast<Discord_String*>(Discord_Alloc(Properties.size * sizeof(Discord_String)));
      size_t i = 0;
  ...
  ActivityInvite::ActivityInvite(ActivityInvite&& other) noexcept : instance_(other.instance_),
                                                                    state_(other.state_)
  {
  ...
  ActivityAssets::ActivityAssets(ActivityAssets&& other) noexcept : instance_(other.instance_),
                                                                    state_(other.state_)
  {
    other.state_ = DiscordObjectState::Invalid;
  }
  ...
  ActivityParty::ActivityParty(ActivityParty&& other) noexcept : instance_(other.instance_),
                                                                 state_(other.state_)
  {
    other.state_ = DiscordObjectState::Invalid;
  }
  ...
  ActivitySecrets::ActivitySecrets(ActivitySecrets&& other) noexcept : instance_(other.instance_),
                                                                       state_(other.state_)
  {
    other.state_ = DiscordObjectState::Invalid;
  }
  ...
  Activity::Activity(Activity&& other) noexcept : instance_(other.instance_), state_(other.state_)
  {
    other.state_ = DiscordObjectState::Invalid;
  }
  ...
  ClientResult::ClientResult(ClientResult&& other) noexcept : instance_(other.instance_),
                                                              state_(other.state_)
  {
    other.state_ = DiscordObjectState::Invalid;
  }
  ...
  AuthorizationCodeChallenge::AuthorizationCodeChallenge(
  AuthorizationCodeChallenge&& other) noexcept : instance_(other.instance_),
                                                 state_(other.state_)
  {
    other.state_ = DiscordObjectState::Invalid;
  }
  ...
  AuthorizationCodeChallenge::AuthorizationCodeChallenge(
  Discord_AuthorizationCodeChallenge instance, DiscordObjectState state)
    : instance_(instance), state_(state)
  {
  }
  AuthorizationCodeChallenge::AuthorizationCodeChallenge()
  ...
  const AuthorizationCodeVerifier AuthorizationCodeVerifier::nullobj{ {},
                                                                      DiscordObjectState::Invalid };
  ...
  Call::Call(Call&& other) noexcept : instance_(other.instance_), state_(other.state_)
  {
    other.state_ = DiscordObjectState::Invalid;
  }
  ...
  void Call::SetOnVoiceStateChangedCallback(discordpp::Call::OnVoiceStateChanged cb)
  {
    ...
    Discord_Call_SetOnVoiceStateChangedCallback(&instance_, cb__native, Tcb__UserData::Free,
                                                cb__userData);
  }
  void Call::SetParticipantChangedCallback(discordpp::Call::OnParticipantChanged cb)
  {
    ...
    Discord_Call_SetParticipantChangedCallback(&instance_, cb__native, Tcb__UserData::Free,
                                               cb__userData);
  }
  ...
  void Call::SetSpeakingStatusChangedCallback(discordpp::Call::OnSpeakingStatusChanged cb)
  {
    ...
    Discord_Call_SetSpeakingStatusChangedCallback(&instance_, cb__native, Tcb__UserData::Free,
                                                  cb__userData);
  }
  void Call::SetStatusChangedCallback(discordpp::Call::OnStatusChanged cb)
  {
    ...
    [](auto status, auto error, auto errorDetail, void* userData__)
    {
      ...
      userData__typed->delegate(static_cast<discordpp::Call::Status>(status),
                                static_cast<discordpp::Call::Error>(error), errorDetail);
    };
    Discord_Call_SetStatusChangedCallback(&instance_, cb__native, Tcb__UserData::Free,
                                          cb__userData);
  }
  ...
  ChannelHandle::ChannelHandle(ChannelHandle&& other) noexcept : instance_(other.instance_),
                                                                 state_(other.state_)
  {
    other.state_ = DiscordObjectState::Invalid;
  }
  ...
  GuildMinimal::GuildMinimal(GuildMinimal&& other) noexcept : instance_(other.instance_),
                                                              state_(other.state_)
  {
    other.state_ = DiscordObjectState::Invalid;
  }
  ...
  GuildChannel::GuildChannel(GuildChannel&& other) noexcept : instance_(other.instance_),
                                                              state_(other.state_)
  {
    other.state_ = DiscordObjectState::Invalid;
  }
  ...
  LinkedLobby::LinkedLobby(LinkedLobby&& other) noexcept : instance_(other.instance_),
                                                           state_(other.state_)
  {
    other.state_ = DiscordObjectState::Invalid;
  }
  ...
  LinkedChannel::LinkedChannel(LinkedChannel&& other) noexcept : instance_(other.instance_),
                                                                 state_(other.state_)
  {
    other.state_ = DiscordObjectState::Invalid;
  }
  ...
  UserHandle::UserHandle(UserHandle&& other) noexcept : instance_(other.instance_),
                                                        state_(other.state_)
  {
    other.state_ = DiscordObjectState::Invalid;
  }
                                    ...
                                    discordpp::UserHandle::AvatarType staticType) const
  {
    ...
    &instance_, static_cast<Discord_UserHandle_AvatarType>(animatedType),
    static_cast<Discord_UserHandle_AvatarType>(staticType), &returnValueNative__);
    std::string returnValue__(reinterpret_cast<char*>(returnValueNative__.ptr),
                              returnValueNative__.size);
    Discord_Free(returnValueNative__.ptr);
    return returnValue__;
  ...
  LobbyHandle::LobbyHandle(LobbyHandle&& other) noexcept : instance_(other.instance_),
                                                           state_(other.state_)
  {
    other.state_ = DiscordObjectState::Invalid;
  }
  ...
  MessageHandle::MessageHandle(MessageHandle&& other) noexcept : instance_(other.instance_),
                                                                 state_(other.state_)
  {
    other.state_ = DiscordObjectState::Invalid;
  }
  ...
  AudioDevice::AudioDevice(AudioDevice&& other) noexcept : instance_(other.instance_),
                                                           state_(other.state_)
  {
    other.state_ = DiscordObjectState::Invalid;
  }
  ...
  Client::Client(Client&& other) noexcept : instance_(other.instance_), state_(other.state_)
  {
    other.state_ = DiscordObjectState::Invalid;
  }
  ...
  void Client::EndCall(uint64_t channelId, discordpp::Client::EndCallCallback callback)
  {
    ...
    Discord_Client_EndCall(&instance_, channelId, callback__native, Tcallback__UserData::Free,
                           callback__userData);
  }
  void Client::EndCalls(discordpp::Client::EndCallsCallback callback)
  {
    ...
    Discord_Client_EndCalls(&instance_, callback__native, Tcallback__UserData::Free,
                            callback__userData);
  }
  discordpp::Call Client::GetCall(uint64_t channelId)
  {
    ...
    discordpp::Call returnValue__(returnValueNative__, returnIsNonNull__ ?
                                                       DiscordObjectState::Owned :
                                                       DiscordObjectState::Invalid);
    return returnValue__;
  }
  ...
  void Client::GetCurrentOutputDevice(discordpp::Client::GetCurrentOutputDeviceCallback cb)
  {
    ...
    Discord_Client_GetCurrentOutputDevice(&instance_, cb__native, Tcb__UserData::Free,
                                          cb__userData);
  }
  ...
  void Client::SetDeviceChangeCallback(discordpp::Client::DeviceChangeCallback callback)
  {
    ...
    Discord_Client_SetDeviceChangeCallback(&instance_, callback__native, Tcallback__UserData::Free,
                                           callback__userData);
  }
  ...
  void Client::SetInputDevice(std::string deviceId, discordpp::Client::SetInputDeviceCallback cb)
  {
    ...
    Discord_Client_SetInputDevice(&instance_, deviceId__str, cb__native, Tcb__UserData::Free,
                                  cb__userData);
  }
  ...
  void Client::SetNoAudioInputCallback(discordpp::Client::NoAudioInputCallback callback)
  {
    ...
    using Tcallback__UserData = TDelegateUserData<std::remove_reference_t<decltype(callback)> >;
    auto callback__userData = new Tcallback__UserData(callback);
    Discord_Client_NoAudioInputCallback callback__native = [](auto inputDetected, void* userData__)
    {
      auto userData__typed = static_cast<Tcallback__UserData*>(userData__);
      userData__typed->delegate(inputDetected);
    };
    Discord_Client_SetNoAudioInputCallback(&instance_, callback__native, Tcallback__UserData::Free,
                                           callback__userData);
  }
  ...
  void Client::SetOutputDevice(std::string deviceId, discordpp::Client::SetOutputDeviceCallback cb)
  {
    ...
    Discord_Client_SetOutputDevice(&instance_, deviceId__str, cb__native, Tcb__UserData::Free,
                                   cb__userData);
  }
  ...
  {
    assert(state_ == DiscordObjectState::Owned);
    Discord_Client_SetThreadPriority(&instance_, static_cast<Discord_Client_Thread>(thread),
                                     priority);
  }
  ...
  discordpp::Client::VoiceParticipantChangedCallback cb)
  {
    ...
    Discord_Client_SetVoiceParticipantChangedCallback(&instance_, cb__native, Tcb__UserData::Free,
                                                      cb__userData);
  }
  ...
  discordpp::Client::UserAudioCapturedCallback capturedCb)
  {
    ...
    Discord_Client_UserAudioReceivedCallback receivedCb__native =
    [](auto userId, auto data, auto samplesPerChannel, auto sampleRate, auto channels,
       auto outShouldMute, void* userData__)
    {
      auto userData__typed = static_cast<TreceivedCb__UserData*>(userData__);
      userData__typed->delegate(userId, data, samplesPerChannel, sampleRate, channels,
                                *outShouldMute);
    };
    ...
    returnIsNonNull__ = Discord_Client_StartCallWithAudioCallbacks(
    &instance_, lobbyId, receivedCb__native, TreceivedCb__UserData::Free, receivedCb__userData,
    capturedCb__native, TcapturedCb__UserData::Free, capturedCb__userData, &returnValueNative__);
    discordpp::Call returnValue__(returnValueNative__, returnIsNonNull__ ?
                                                       DiscordObjectState::Owned :
                                                       DiscordObjectState::Invalid);
    return returnValue__;
                         ...
                         discordpp::Client::AuthorizationCallback callback)
  {
    ...
    [](auto result, auto code, auto redirectUri, void* userData__)
    {
      ...
      userData__typed->delegate(std::move(result__obj), std::move(code__str),
                                std::move(redirectUri__str));
      Discord_Free(redirectUri.ptr);
      Discord_Free(code.ptr);
    };
    Discord_Client_Authorize(&instance_, args.instance(), callback__native,
                             Tcallback__UserData::Free, callback__userData);
  }
                                ...
                                discordpp::Client::FetchCurrentUserCallback callback)
  {
    ...
    &instance_, static_cast<Discord_AuthorizationTokenType>(tokenType), token__str,
    callback__native, Tcallback__UserData::Free, callback__userData);
  }
                                   ...
                                   discordpp::Client::TokenExchangeCallback callback)
  {
    ...
    using Tcallback__UserData = TDelegateUserData<std::remove_reference_t<decltype(callback)> >;
    auto callback__userData = new Tcallback__UserData(callback);
    Discord_Client_TokenExchangeCallback callback__native =
    [](auto result, auto accessToken, auto refreshToken, auto tokenType, auto expiresIn,
       auto scopes, void* userData__)
    {
      auto userData__typed = static_cast<Tcallback__UserData*>(userData__);
      discordpp::ClientResult result__obj(*result, DiscordObjectState::Owned);
      ...
      userData__typed->delegate(
      std::move(result__obj), std::move(accessToken__str), std::move(refreshToken__str),
      static_cast<discordpp::AuthorizationTokenType>(tokenType), expiresIn, std::move(scopes__str));
      Discord_Free(scopes.ptr);
      Discord_Free(refreshToken.ptr);
      Discord_Free(accessToken.ptr);
    };
    Discord_Client_GetProvisionalToken(
    &instance_, applicationId,
    static_cast<Discord_AuthenticationExternalAuthType>(externalAuthType), externalAuthToken__str,
    callback__native, Tcallback__UserData::Free, callback__userData);
  }
  void Client::GetToken(uint64_t applicationId, std::string const& code,
                        std::string const& codeVerifier, std::string const& redirectUri,
                        discordpp::Client::TokenExchangeCallback callback)
  {
    assert(state_ == DiscordObjectState::Owned);
    ...
    Discord_Client_GetToken(&instance_, applicationId, code__str, codeVerifier__str,
                            redirectUri__str, callback__native, Tcallback__UserData::Free,
                            callback__userData);
  }
                                  ...
                                  discordpp::Client::TokenExchangeCallback callback)
  {
    ...
    Discord_Client_GetTokenFromDevice(&instance_, args.instance(), callback__native,
                                      Tcallback__UserData::Free, callback__userData);
  }
  void Client::GetTokenFromDeviceProvisionalMerge(
  discordpp::DeviceAuthorizationArgs args,
  discordpp::AuthenticationExternalAuthType externalAuthType, std::string const& externalAuthToken,
  discordpp::Client::TokenExchangeCallback callback)
  {
    assert(state_ == DiscordObjectState::Owned);
    Discord_String externalAuthToken__str{ (uint8_t*)(externalAuthToken.data()),
    ...
    &instance_, args.instance(),
    static_cast<Discord_AuthenticationExternalAuthType>(externalAuthType), externalAuthToken__str,
    callback__native, Tcallback__UserData::Free, callback__userData);
  }
  void Client::GetTokenFromProvisionalMerge(
  uint64_t applicationId, std::string const& code, std::string const& codeVerifier,
  std::string const& redirectUri, discordpp::AuthenticationExternalAuthType externalAuthType,
  std::string const& externalAuthToken, discordpp::Client::TokenExchangeCallback callback)
  {
    assert(state_ == DiscordObjectState::Owned);
    Discord_String code__str{ (uint8_t*)(code.data()), code.size() };
    ...
    &instance_, applicationId, code__str, codeVerifier__str, redirectUri__str,
    static_cast<Discord_AuthenticationExternalAuthType>(externalAuthType), externalAuthToken__str,
    callback__native, Tcallback__UserData::Free, callback__userData);
  }
  ...
  void Client::RefreshToken(uint64_t applicationId, std::string const& refreshToken,
                            discordpp::Client::TokenExchangeCallback callback)
  {
    assert(state_ == DiscordObjectState::Owned);
    ...
    Discord_Client_RefreshToken(&instance_, applicationId, refreshToken__str, callback__native,
                                Tcallback__UserData::Free, callback__userData);
  }
  ...
  discordpp::Client::AuthorizeDeviceScreenClosedCallback cb)
  {
    ...
    Discord_Client_SetAuthorizeDeviceScreenClosedCallback(&instance_, cb__native,
                                                          Tcb__UserData::Free, cb__userData);
  }
  ...
  void Client::SetTokenExpirationCallback(discordpp::Client::TokenExpirationCallback callback)
  {
    ...
    Discord_Client_SetTokenExpirationCallback(&instance_, callback__native,
                                              Tcallback__UserData::Free, callback__userData);
  }
  void Client::UpdateProvisionalAccountDisplayName(
  std::string const& name, discordpp::Client::UpdateProvisionalAccountDisplayNameCallback callback)
  {
    assert(state_ == DiscordObjectState::Owned);
    Discord_String name__str{ (uint8_t*)(name.data()), name.size() };
    using Tcallback__UserData = TDelegateUserData<std::remove_reference_t<decltype(callback)> >;
  ...
  void Client::UpdateToken(discordpp::AuthorizationTokenType tokenType, std::string token,
                           discordpp::Client::UpdateTokenCallback callback)
  {
    assert(state_ == DiscordObjectState::Owned);
    Discord_String token__str{ (uint8_t*)(token.data()), token.size() };
    ...
    Discord_Client_UpdateToken(&instance_, static_cast<Discord_AuthorizationTokenType>(tokenType),
                               token__str, callback__native, Tcallback__UserData::Free,
                               callback__userData);
  }
  ...
  void Client::DeleteUserMessage(uint64_t recipientId, uint64_t messageId,
                                 discordpp::Client::DeleteUserMessageCallback cb)
  {
    assert(state_ == DiscordObjectState::Owned);
    using Tcb__UserData = TDelegateUserData<std::remove_reference_t<decltype(cb)> >;
    ...
    Discord_Client_DeleteUserMessage(&instance_, recipientId, messageId, cb__native,
                                     Tcb__UserData::Free, cb__userData);
  }
  void Client::EditUserMessage(uint64_t recipientId, uint64_t messageId, std::string const& content,
                               discordpp::Client::EditUserMessageCallback cb)
  {
    assert(state_ == DiscordObjectState::Owned);
    ...
    Discord_Client_EditUserMessage(&instance_, recipientId, messageId, content__str, cb__native,
                                   Tcb__UserData::Free, cb__userData);
  }
  ...
  discordpp::Client::OpenMessageInDiscordCallback callback)
  {
    ...
    using Tcallback__UserData = TDelegateUserData<std::remove_reference_t<decltype(callback)> >;
    auto callback__userData = new Tcallback__UserData(callback);
    Discord_Client_OpenMessageInDiscordCallback callback__native = [](auto result, void* userData__)
    {
      auto userData__typed = static_cast<Tcallback__UserData*>(userData__);
      discordpp::ClientResult result__obj(*result, DiscordObjectState::Owned);
      userData__typed->delegate(std::move(result__obj));
    ...
    &instance_, messageId, provisionalUserMergeRequiredCallback__native,
    TprovisionalUserMergeRequiredCallback__UserData::Free,
    provisionalUserMergeRequiredCallback__userData, callback__native, Tcallback__UserData::Free,
    callback__userData);
  }
  void Client::SendLobbyMessage(uint64_t lobbyId, std::string const& content,
                                discordpp::Client::SendUserMessageCallback cb)
  {
    assert(state_ == DiscordObjectState::Owned);
    Discord_String content__str{ (uint8_t*)(content.data()), content.size() };
    ...
    Discord_Client_SendLobbyMessage(&instance_, lobbyId, content__str, cb__native,
                                    Tcb__UserData::Free, cb__userData);
  }
  void Client::SendLobbyMessageWithMetadata(
  uint64_t lobbyId, std::string const& content,
  std::unordered_map<std::string, std::string> const& metadata,
  discordpp::Client::SendUserMessageCallback cb)
  {
    assert(state_ == DiscordObjectState::Owned);
    ...
    Discord_Client_SendLobbyMessageWithMetadata(&instance_, lobbyId, content__str,
                                                metadata__convert.Properties, cb__native,
                                                Tcb__UserData::Free, cb__userData);
  }
  void Client::SendUserMessage(uint64_t recipientId, std::string const& content,
                               discordpp::Client::SendUserMessageCallback cb)
  {
    assert(state_ == DiscordObjectState::Owned);
    Discord_String content__str{ (uint8_t*)(content.data()), content.size() };
    ...
    Discord_Client_SendUserMessage(&instance_, recipientId, content__str, cb__native,
                                   Tcb__UserData::Free, cb__userData);
  }
  void Client::SendUserMessageWithMetadata(
  uint64_t recipientId, std::string const& content,
  std::unordered_map<std::string, std::string> const& metadata,
  discordpp::Client::SendUserMessageCallback cb)
  {
    assert(state_ == DiscordObjectState::Owned);
    ...
    Discord_Client_SendUserMessageWithMetadata(&instance_, recipientId, content__str,
                                               metadata__convert.Properties, cb__native,
                                               Tcb__UserData::Free, cb__userData);
  }
  void Client::SetMessageCreatedCallback(discordpp::Client::MessageCreatedCallback cb)
  {
    ...
    Discord_Client_SetMessageCreatedCallback(&instance_, cb__native, Tcb__UserData::Free,
                                             cb__userData);
  }
  void Client::SetMessageDeletedCallback(discordpp::Client::MessageDeletedCallback cb)
  {
    ...
    Discord_Client_SetMessageDeletedCallback(&instance_, cb__native, Tcb__UserData::Free,
                                             cb__userData);
  }
  void Client::SetMessageUpdatedCallback(discordpp::Client::MessageUpdatedCallback cb)
  {
    ...
    Discord_Client_SetMessageUpdatedCallback(&instance_, cb__native, Tcb__UserData::Free,
                                             cb__userData);
  }
                              ...
                              discordpp::LoggingSeverity minSeverity)
  {
    ...
    using Tcallback__UserData = TDelegateUserData<std::remove_reference_t<decltype(callback)> >;
    auto callback__userData = new Tcallback__UserData(callback);
    Discord_Client_LogCallback callback__native = [](auto message, auto severity, void* userData__)
    {
      auto userData__typed = static_cast<Tcallback__UserData*>(userData__);
    ...
    Discord_Client_AddLogCallback(&instance_, callback__native, Tcallback__UserData::Free,
                                  callback__userData,
                                   ...
                                   discordpp::LoggingSeverity minSeverity)
  {
    ...
    Discord_Client_AddVoiceLogCallback(&instance_, callback__native, Tcallback__UserData::Free,
                                       callback__userData,
                                       static_cast<Discord_LoggingSeverity>(minSeverity));
  }
  void Client::Connect()
  ...
  bool Client::SetLogDir(std::string const& path, discordpp::LoggingSeverity minSeverity)
  {
    ...
    returnValue__ = Discord_Client_SetLogDir(&instance_, path__str,
                                             static_cast<Discord_LoggingSeverity>(minSeverity));
    return returnValue__;
  }
  void Client::SetStatusChangedCallback(discordpp::Client::OnStatusChanged cb)
  {
    ...
    [](auto status, auto error, auto errorDetail, void* userData__)
    {
      ...
      userData__typed->delegate(static_cast<discordpp::Client::Status>(status),
                                static_cast<discordpp::Client::Error>(error), errorDetail);
    };
    Discord_Client_SetStatusChangedCallback(&instance_, cb__native, Tcb__UserData::Free,
                                            cb__userData);
  }
  void Client::SetVoiceLogDir(std::string const& path, discordpp::LoggingSeverity minSeverity)
  {
    assert(state_ == DiscordObjectState::Owned);
    Discord_String path__str{ (uint8_t*)(path.data()), path.size() };
    Discord_Client_SetVoiceLogDir(&instance_, path__str,
                                  static_cast<Discord_LoggingSeverity>(minSeverity));
  }
                                 ...
                                 discordpp::Client::CreateOrJoinLobbyCallback callback)
  {
    ...
    Discord_Client_CreateOrJoinLobby(&instance_, secret__str, callback__native,
                                     Tcallback__UserData::Free, callback__userData);
  }
  ...
  discordpp::Client::CreateOrJoinLobbyCallback callback)
  {
    ...
    &instance_, secret__str, lobbyMetadata__convert.Properties, memberMetadata__convert.Properties,
    callback__native, Tcallback__UserData::Free, callback__userData);
  }
  void Client::GetGuildChannels(uint64_t guildId, discordpp::Client::GetGuildChannelsCallback cb)
  {
    ...
    Discord_Client_GetGuildChannels(&instance_, guildId, cb__native, Tcb__UserData::Free,
                                    cb__userData);
  }
  ...
  void Client::GetUserGuilds(discordpp::Client::GetUserGuildsCallback cb)
  {
    ...
    using Tcb__UserData = TDelegateUserData<std::remove_reference_t<decltype(cb)> >;
    auto cb__userData = new Tcb__UserData(cb);
    Discord_Client_GetUserGuildsCallback cb__native = [](auto result, auto guilds, void* userData__)
    {
      auto userData__typed = static_cast<Tcb__UserData*>(userData__);
      discordpp::ClientResult result__obj(*result, DiscordObjectState::Owned);
  ...
  void Client::LeaveLobby(uint64_t lobbyId, discordpp::Client::LeaveLobbyCallback callback)
  {
    ...
    Discord_Client_LeaveLobby(&instance_, lobbyId, callback__native, Tcallback__UserData::Free,
                              callback__userData);
  }
  void Client::LinkChannelToLobby(uint64_t lobbyId, uint64_t channelId,
                                  discordpp::Client::LinkOrUnlinkChannelCallback callback)
  {
    assert(state_ == DiscordObjectState::Owned);
    using Tcallback__UserData = TDelegateUserData<std::remove_reference_t<decltype(callback)> >;
    auto callback__userData = new Tcallback__UserData(callback);
    Discord_Client_LinkOrUnlinkChannelCallback callback__native = [](auto result, void* userData__)
    {
      auto userData__typed = static_cast<Tcallback__UserData*>(userData__);
      discordpp::ClientResult result__obj(*result, DiscordObjectState::Owned);
      userData__typed->delegate(std::move(result__obj));
    };
    Discord_Client_LinkChannelToLobby(&instance_, lobbyId, channelId, callback__native,
                                      Tcallback__UserData::Free, callback__userData);
  }
  void Client::SetLobbyCreatedCallback(discordpp::Client::LobbyCreatedCallback cb)
  {
    ...
    Discord_Client_SetLobbyCreatedCallback(&instance_, cb__native, Tcb__UserData::Free,
                                           cb__userData);
  }
  void Client::SetLobbyDeletedCallback(discordpp::Client::LobbyDeletedCallback cb)
  {
    ...
    Discord_Client_SetLobbyDeletedCallback(&instance_, cb__native, Tcb__UserData::Free,
                                           cb__userData);
  }
  void Client::SetLobbyMemberAddedCallback(discordpp::Client::LobbyMemberAddedCallback cb)
  {
    ...
    Discord_Client_SetLobbyMemberAddedCallback(&instance_, cb__native, Tcb__UserData::Free,
                                               cb__userData);
  }
  void Client::SetLobbyMemberRemovedCallback(discordpp::Client::LobbyMemberRemovedCallback cb)
  {
    ...
    Discord_Client_SetLobbyMemberRemovedCallback(&instance_, cb__native, Tcb__UserData::Free,
                                                 cb__userData);
  }
  void Client::SetLobbyMemberUpdatedCallback(discordpp::Client::LobbyMemberUpdatedCallback cb)
  {
    ...
    Discord_Client_SetLobbyMemberUpdatedCallback(&instance_, cb__native, Tcb__UserData::Free,
                                                 cb__userData);
  }
  void Client::SetLobbyUpdatedCallback(discordpp::Client::LobbyUpdatedCallback cb)
  {
    ...
    Discord_Client_SetLobbyUpdatedCallback(&instance_, cb__native, Tcb__UserData::Free,
                                           cb__userData);
  }
                                      ...
                                      discordpp::Client::LinkOrUnlinkChannelCallback callback)
  {
    ...
    Discord_Client_UnlinkChannelFromLobby(&instance_, lobbyId, callback__native,
                                          Tcallback__UserData::Free, callback__userData);
  }
                                    ...
                                    discordpp::Client::AcceptActivityInviteCallback cb)
  {
    ...
    Discord_Client_AcceptActivityInvite(&instance_, invite.instance(), cb__native,
                                        Tcb__UserData::Free, cb__userData);
  }
  ...
  void Client::SendActivityInvite(uint64_t userId, std::string const& content,
                                  discordpp::Client::SendActivityInviteCallback cb)
  {
    assert(state_ == DiscordObjectState::Owned);
    Discord_String content__str{ (uint8_t*)(content.data()), content.size() };
    ...
    Discord_Client_SendActivityInvite(&instance_, userId, content__str, cb__native,
                                      Tcb__UserData::Free, cb__userData);
  }
                                       ...
                                       discordpp::Client::SendActivityInviteCallback cb)
  {
    ...
    Discord_Client_SendActivityJoinRequest(&instance_, userId, cb__native, Tcb__UserData::Free,
                                           cb__userData);
  }
                                            ...
                                            discordpp::Client::SendActivityInviteCallback cb)
  {
    ...
    Discord_Client_SendActivityJoinRequestReply(&instance_, invite.instance(), cb__native,
                                                Tcb__UserData::Free, cb__userData);
  }
  void Client::SetActivityInviteCreatedCallback(discordpp::Client::ActivityInviteCallback cb)
  {
    ...
    Discord_Client_SetActivityInviteCreatedCallback(&instance_, cb__native, Tcb__UserData::Free,
                                                    cb__userData);
  }
  void Client::SetActivityInviteUpdatedCallback(discordpp::Client::ActivityInviteCallback cb)
  {
    ...
    Discord_Client_SetActivityInviteUpdatedCallback(&instance_, cb__native, Tcb__UserData::Free,
                                                    cb__userData);
  }
  void Client::SetActivityJoinCallback(discordpp::Client::ActivityJoinCallback cb)
  {
    ...
    Discord_Client_SetActivityJoinCallback(&instance_, cb__native, Tcb__UserData::Free,
                                           cb__userData);
  }
                               ...
                               discordpp::Client::UpdateStatusCallback callback)
  {
    ...
    Discord_Client_SetOnlineStatus(&instance_, static_cast<Discord_StatusType>(status),
                                   callback__native, Tcallback__UserData::Free, callback__userData);
  }
                                  ...
                                  discordpp::Client::UpdateRichPresenceCallback cb)
  {
    ...
    Discord_Client_UpdateRichPresence(&instance_, activity.instance(), cb__native,
                                      Tcb__UserData::Free, cb__userData);
  }
                                          ...
                                          discordpp::Client::UpdateRelationshipCallback cb)
  {
    ...
    Discord_Client_AcceptDiscordFriendRequest(&instance_, userId, cb__native, Tcb__UserData::Free,
                                              cb__userData);
  }
                                       ...
                                       discordpp::Client::UpdateRelationshipCallback cb)
  {
    ...
    Discord_Client_AcceptGameFriendRequest(&instance_, userId, cb__native, Tcb__UserData::Free,
                                           cb__userData);
  }
                                          ...
                                          discordpp::Client::UpdateRelationshipCallback cb)
  {
    ...
    Discord_Client_CancelDiscordFriendRequest(&instance_, userId, cb__native, Tcb__UserData::Free,
                                              cb__userData);
  }
                                       ...
                                       discordpp::Client::UpdateRelationshipCallback cb)
  {
    ...
    Discord_Client_CancelGameFriendRequest(&instance_, userId, cb__native, Tcb__UserData::Free,
                                           cb__userData);
  }
                                          ...
                                          discordpp::Client::UpdateRelationshipCallback cb)
  {
    ...
    Discord_Client_RejectDiscordFriendRequest(&instance_, userId, cb__native, Tcb__UserData::Free,
                                              cb__userData);
  }
                                       ...
                                       discordpp::Client::UpdateRelationshipCallback cb)
  {
    ...
    Discord_Client_RejectGameFriendRequest(&instance_, userId, cb__native, Tcb__UserData::Free,
                                           cb__userData);
  }
                                          ...
                                          discordpp::Client::UpdateRelationshipCallback cb)
  {
    ...
    Discord_Client_RemoveDiscordAndGameFriend(&instance_, userId, cb__native, Tcb__UserData::Free,
                                              cb__userData);
  }
  void Client::RemoveGameFriend(uint64_t userId, discordpp::Client::UpdateRelationshipCallback cb)
  {
    ...
    Discord_Client_RemoveGameFriend(&instance_, userId, cb__native, Tcb__UserData::Free,
                                    cb__userData);
  }
                                        ...
                                        discordpp::Client::SendFriendRequestCallback cb)
  {
    ...
    Discord_Client_SendDiscordFriendRequest(&instance_, username__str, cb__native,
                                            Tcb__UserData::Free, cb__userData);
  }
                                            ...
                                            discordpp::Client::UpdateRelationshipCallback cb)
  {
    ...
    Discord_Client_SendDiscordFriendRequestById(&instance_, userId, cb__native, Tcb__UserData::Free,
                                                cb__userData);
  }
                                     ...
                                     discordpp::Client::SendFriendRequestCallback cb)
  {
    ...
    Discord_Client_SendGameFriendRequest(&instance_, username__str, cb__native, Tcb__UserData::Free,
                                         cb__userData);
  }
                                         ...
                                         discordpp::Client::UpdateRelationshipCallback cb)
  {
    ...
    Discord_Client_SendGameFriendRequestById(&instance_, userId, cb__native, Tcb__UserData::Free,
                                             cb__userData);
  }
  void Client::SetRelationshipCreatedCallback(discordpp::Client::RelationshipCreatedCallback cb)
  {
    ...
    Discord_Client_SetRelationshipCreatedCallback(&instance_, cb__native, Tcb__UserData::Free,
                                                  cb__userData);
  }
  void Client::SetRelationshipDeletedCallback(discordpp::Client::RelationshipDeletedCallback cb)
  {
    ...
    Discord_Client_SetRelationshipDeletedCallback(&instance_, cb__native, Tcb__UserData::Free,
                                                  cb__userData);
  }
  ...
  uint64_t applicationId, discordpp::Client::GetDiscordClientConnectedUserCallback callback) const
  {
    assert(state_ == DiscordObjectState::Owned);
    using Tcallback__UserData = TDelegateUserData<std::remove_reference_t<decltype(callback)> >;
    auto callback__userData = new Tcallback__UserData(callback);
    ...
    Discord_Client_GetDiscordClientConnectedUser(&instance_, applicationId, callback__native,
                                                 Tcallback__UserData::Free, callback__userData);
  }
  ...
  void Client::SetUserUpdatedCallback(discordpp::Client::UserUpdatedCallback cb)
  {
    ...
    Discord_Client_SetUserUpdatedCallback(&instance_, cb__native, Tcb__UserData::Free,
                                          cb__userData);
  }
  ...
  CallInfoHandle::CallInfoHandle(CallInfoHandle&& other) noexcept : instance_(other.instance_),
                                                                    state_(other.state_)
  {
    other.state_ = DiscordObjectState::Invalid;
  }
